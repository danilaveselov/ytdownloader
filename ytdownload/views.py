    ...
    if request.method == 'POST' and 'video' in request.POST:
        ...
        if form.is_valid():
            ...
            with io.BytesIO() as file_buffer:
                YouTube(video_link).streams.filter(progressive=True, file_extension='mp4').order_by(
                    'resolution').last().stream_to_buffer(file_buffer)
                response = HttpResponse(
                    file_buffer.getvalue(), content_type='video/mp4')
        ...
        if form.is_valid():
            video_link = form.cleaned_data['video_link']
            with io.BytesIO() as file_buffer:
                YouTube(video_link).streams.get_audio_only(
                ).stream_to_buffer(file_buffer)
                response = HttpResponse(
                    file_buffer.getvalue(), content_type='audio/mp3')
